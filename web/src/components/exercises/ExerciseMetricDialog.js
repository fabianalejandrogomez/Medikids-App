...
class ExerciseMetricDialog extends Component {
    ...
    handleNameChange = (event, value) => {
        ...
        if (nameList.find(n => n.toLowerCase() === value.toLowerCase())) {
            this.setState({
                validationErrors: { name: 'Este nombre ya esta en uso.' },
            });
        }
        else {
    ...
    handleSaveClick = (result) => {
        if (this.state.metric.name === '') {
            this.setState({
                validationErrors: { name: 'La metrica debe tener un nombre.' },
            });
        }
        else {
    ...
    render() {
        return (
            <Dialog
                title={(this.props.intent === INTENTS.ADD ? 'Agregar' : 'Editar') + ' Metrica'} 
                actions={
                    <div>
                        <FlatButton
                            label="Cancelar"
            ...
            >
                ...
                <TextField
                    hintText="ejemplo 'Kg'"
                    floatingLabelText="(Opcional) Unidad de Medida"
