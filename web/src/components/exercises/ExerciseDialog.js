...
class ExerciseDialog extends Component {
    ...
    handleSaveClick = () => {
        this.setState({
            validationErrors: { 
                name: this.state.exercise.name === '' ? 'La Medicion Debe Tener un Nombre.' : '',
                type: this.state.exercise.type === '' ? 'Un tipo debe ser seleccionado.' : '',
                url: this.state.exercise.url && !validateUrl(this.state.exercise.url) ? 'La URL debe ser valida.' : '',
            },
        }, () => {
            if (Object.keys(this.state.validationErrors).find(e => this.state.validationErrors[e] !== '') === undefined) {
                ...
                if (this.props.intent === INTENTS.EDIT) {
                    ...
                    .then((response) => {
                        this.handleApiSuccess('Medicion Actualizada \'' + response.data.name + '\'.');
                    }, (error) => {
                        this.handleApiError('Error Actualizando Medicion \'' + this.state.exercise.name + '\': ' + error);
                    });
                }
                else {
                    this.props.addExercise(this.state.exercise)
                    .then((response) => {
                        this.handleApiSuccess('Medicion Agregada \'' + response.data.name + '\'.');
                    }, (error) => {
                        this.handleApiError('Error Agregando Medicion \'' + this.state.exercise.name + '\': ' + error);
                    });
                }
            }
        });
        ...
        return (
            <div>
                ...
                >
                    ...
                    <TextField
                        hintText=""
                        floatingLabelText="(Opcional) Url"
