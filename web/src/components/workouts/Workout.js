...
class Workout extends Component {
    ...
    componentWillMount = () => {
        this.props.setTitle('Eventos');
        this.fetchWorkout();
    }

    getWorkout = (props = this.props) => {
        let workout = props && props.workouts ? props.workouts.find(w => w._id === props.match.params.id) : undefined;
    ...
    fetchWorkout = () => {
        ...
        .then(response => {
            this.setState({ 
                workout: response.data.find(w => w._id === this.props.match.params.id)
            }, () => {
    ...
    handleWorkoutHistoryDelete = () => {
        ...
        return new Promise((resolve, reject) => {
            this.props.deleteWorkoutHistory(workout._id)
            .then(response => {
    ...
    handleWorkoutReset = () => {
        ...
        return this.handleWorkoutChange(workout, 'Evento Reseteado \'' + workout.routine.name + '\'.');
    }
    ...
    handleWorkoutReschedule = (datetime = { date: undefined, time: undefined}) => {
        ...
        return this.handleWorkoutChange(workout, 'Evento Reagendado \'' + workout.routine.name + '\' for ' + moment(scheduledTime).calendar() + '.');
    }
    ...
    handleWorkoutExerciseChange = (exercise) => {
        ...
        workout.routine.exercises = workout.routine.exercises.map(e => {
            return e.sequence === exercise.sequence && e._id === exercise._id ? exercise : e;
        });
    ...
    handleWorkoutChange = (workout, notify = undefined) => {
        return new Promise((resolve, reject) => {
            ...
            .then(response => {
                ...
                this.props.showSnackbar('Error Actualizando Evento \'' + workout.routine.name + '\'.');
                reject(error);
            });
        });
    }
    ...
    handleWorkoutComplete = (workout) => {
        return new Promise((resolve, reject) => {
            ...
            .then(response => {
                this.props.showSnackbar('Evento Completado \'' + workout.routine.name + '\'.');
                resolve(response);
            }, error => {
                this.props.showSnackbar('Error completando Evento \'' + workout.routine.name + '\'.');
                reject(error);
            })
            .then(() => {
    ...
    handleWorkoutStart = (workout) => {
        return new Promise((resolve, reject) => {
            ...
            .then(response => {
                this.props.showSnackbar('Evento Comenzado \'' + workout.routine.name + '\'.');
                resolve(response);
            }, error => {
                this.props.showSnackbar('Error comenzando Evento \'' + workout.routine.name + '\'.');
            });
        });
    }
    ...
    handleWorkoutDelete = () => {
        ...
        return new Promise((resolve, reject) => {
            ...
            .then(response => {
                this.props.showSnackbar('Evento Borrado \'' + workout.routine.name + '\'.');
                this.navigate('../');
                resolve(response);
            }, error => {
                this.props.showSnackbar('Error borrando evento \'' + workout.routine.name + '\'.');
                reject(error);
            });
        });
    }
