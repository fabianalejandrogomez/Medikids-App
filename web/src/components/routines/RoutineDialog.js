...
class RoutineDialog extends Component {
    ...
    handleNameChange = (event, value) => {
        ...
        if (nameList.find(n => n.toLowerCase() === value.toLowerCase())) {
            this.setState(prevState => ({
                validationErrors: { ...prevState.validationErrors, name: 'Este nombre ya se encuentra en uso.' }, 
            }));
        }
        else {
            this.setState(prevState => ({
    ...
    handleSaveClick = () => {
        this.setState({
            validationErrors: { 
                name: this.state.routine.name === '' ? 'El Estudio debe tener un nombre.' : '',
                color: this.state.routine.color === undefined ? 'El Estudio debe tener asignado un color.' : '',
            },
        }, () => {
            if (Object.keys(this.state.validationErrors).find(e => this.state.validationErrors[e] !== '') === undefined) {
                ...
                if (this.props.intent === INTENTS.EDIT) {
                    ...
                    .then((response) => {
                        this.handleApiSuccess('Estudio Actualizado \'' + response.data.name + '\'.');
                    }, (error) => {
                        this.handleApiError('Error actualizando el Estudio \'' + routine.name + ': ' + error);
                    });
                }
                else {
                    this.props.addRoutine(routine)
                    .then((response) => {
                        this.handleApiSuccess('Estudio Agregado \'' + response.data.name + '\'.');
                    }, (error) => {
                        this.handleApiError('Error agregando Estudio \'' + routine.name + ': ' + error);
                    });
                }
            }
        });
